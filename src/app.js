"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var body_parser_1 = require("body-parser");
var database_1 = require("./config/database");
var path_1 = require("path");
var app = (0, express_1.default)();
var port = 3000;
app.use(express_1.default.static(path_1.default.join(__dirname, "../public")));
app.use(body_parser_1.default.json());
app.get("/users", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var db, rows, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, (0, database_1.openDb)()];
            case 1:
                db = _a.sent();
                return [4 /*yield*/, db.all("SELECT * FROM users ORDER BY id DESC")];
            case 2:
                rows = _a.sent();
                res.json(rows);
                return [3 /*break*/, 4];
            case 3:
                error_1 = _a.sent();
                console.error("Error retrieving users:", error_1);
                res.status(500).json({ message: "Failed to retrieve users" });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
app.post("/users", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name, age, db, rows, error_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, name = _a.name, age = _a.age;
                if (!name || !age) {
                    return [2 /*return*/, res.status(400).json({ message: "Name and age are required" })];
                }
                _b.label = 1;
            case 1:
                _b.trys.push([1, 5, , 6]);
                return [4 /*yield*/, (0, database_1.openDb)()];
            case 2:
                db = _b.sent();
                return [4 /*yield*/, db.run("INSERT INTO users (name, age) VALUES (?, ?)", [name, age])];
            case 3:
                _b.sent();
                return [4 /*yield*/, db.all("SELECT * FROM users ORDER BY id DESC")];
            case 4:
                rows = _b.sent();
                res.status(201).json(rows);
                return [3 /*break*/, 6];
            case 5:
                error_2 = _b.sent();
                console.error("Error creating user:", error_2);
                res.status(500).json({ message: "Failed to create user" });
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); });
app.delete("/users/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, db, result, rows, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 5, , 6]);
                return [4 /*yield*/, (0, database_1.openDb)()];
            case 2:
                db = _a.sent();
                return [4 /*yield*/, db.run("DELETE FROM users WHERE id = ?", [id])];
            case 3:
                result = _a.sent();
                if (result.changes === 0) {
                    return [2 /*return*/, res.status(404).json({ message: "User not found" })];
                }
                return [4 /*yield*/, db.all("SELECT * FROM users ORDER BY id DESC")];
            case 4:
                rows = _a.sent();
                res.status(200).json(rows);
                return [3 /*break*/, 6];
            case 5:
                error_3 = _a.sent();
                console.error("Error deleting user:", error_3);
                res.status(500).json({ message: "Failed to delete user" });
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); });
app.put("/users/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, _a, name, age, db, result, rows, error_4;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                id = req.params.id;
                _a = req.body, name = _a.name, age = _a.age;
                if (!name || !age) {
                    return [2 /*return*/, res.status(400).json({ message: "Name and age are required" })];
                }
                _b.label = 1;
            case 1:
                _b.trys.push([1, 5, , 6]);
                return [4 /*yield*/, (0, database_1.openDb)()];
            case 2:
                db = _b.sent();
                return [4 /*yield*/, db.run("UPDATE users SET name = ?, age = ? WHERE id = ?", [name, age, id])];
            case 3:
                result = _b.sent();
                if (result.changes === 0) {
                    return [2 /*return*/, res.status(404).json({ message: "User not found" })];
                }
                return [4 /*yield*/, db.all("SELECT * FROM users ORDER BY id DESC")];
            case 4:
                rows = _b.sent();
                res.status(200).json(rows);
                return [3 /*break*/, 6];
            case 5:
                error_4 = _b.sent();
                console.error("Error updating user:", error_4);
                res.status(500).json({ message: "Failed to update user" });
                return [3 /*break*/, 6];
            case 6: return [2 /*return*/];
        }
    });
}); });
app.listen(port, function () {
    console.log("Server running at http://localhost:".concat(port));
});
